...
int main(int argc, char *argv[]) {
	...
	if(argc > 4) {	
		printf("Proper usage: otp_enc [file] [key file] [port] \n");
		exit(1);
	}
	else {
	...
	char cypherText[msgLen+1];
	bzero(cypherText, msgLen);
	cypherText[msgLen] = '\n';
	...
	setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, &MAX_SIZE, sizeof(int));
	...
	if(read(sockfd, cypherText, msgLen) < 0) {
		perror("reading response");
		exit(1);
	}
